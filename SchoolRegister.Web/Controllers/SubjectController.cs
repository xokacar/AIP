        ...
        public IActionResult Index(string filterValue = null)
        {
            ...
            else if (_userManager.IsInRoleAsync(user, "Teacher").Result)
            {
                if (user is Teacher teacher)
                {
                    ...
                    {
                        var invokedFilterExpr = Expression
                                    .Invoke(filterExpression, filterTeacherExpression.Parameters);
                        finalFilterBody = Expression
                                    .AndAlso(filterTeacherExpression.Body, invokedFilterExpr);
                    }
                    else
                        finalFilterBody = filterTeacherExpression.Body;
                    var finalFilterExpression = Expression
                                .Lambda<Func<Subject, bool>>(finalFilterBody, filterTeacherExpression.Parameters);
